...
function calculateMoyennetechnique() {
  ...
  if ([syntheseMeca, syntheseElec].every(n => n === null)) {
    result.textContent = "Moyenne Technique:";
    return;
  }
  ...
  result.textContent = `Moyenne Technique: ${moyenne.toFixed(2)}`;
  result.classList.add("gradient-text");
}
...
function calculateMoyenneScoreTech() {
  try {
    ...
    let totalMoyenne = (technique * 3) + (math * 3) + (physique * 3) + 
                      anglais + francais + arabe + philosophie + 
                      informatique + tp + optionContribution;
    ...
    let score = 4 * moyenne + 1.5 * technique + 1.5 * math + physique + 
               francais + anglais;
