...
function calculateMoyennetechnique() {
  ...
  if (synthese === null) {
    result.textContent = "Moyenne S.N:";
    return;
  }
  ...
  const moyenne = total / weights;
  result.textContent = `Moyenne S.N: ${moyenne.toFixed(2)}`;
  result.classList.add("gradient-text");
}
...
function calculateMoyenneinformatique() {
  ...
  if (isNaN(synthese)) {
    result.textContent = "Moyenne Sp.sportive:";
    result.classList.remove("error");
    result.classList.add("gradient-text");
    return;
  }
  ...
  const moyenne = (orale + controle + synthese * 2) / elementsCount;
  result.textContent = `Moyenne Sp.sportive: ${moyenne.toFixed(2)}`;
  result.classList.remove("error");
  result.classList.add("gradient-text");
}

if (document.getElementById("oraleinfo")) {
  document.getElementById("oraleinfo").addEventListener("input", calculateMoyenneinformatique);
}
...
function calculateMoyennesport() {
  const controle1Input = document.getElementById("controle1sport");
  const controle2Input = document.getElementById("controle2sport");
  const controle3Input = document.getElementById("controle3sport");
  const syntheseInput = document.getElementById("synthesesport");
  const result = document.getElementById("moyensport");

  if (!controle1Input || !controle2Input || !controle3Input || !syntheseInput || !result) return;

  const controle1 = controle1Input.value.trim() === "" ? null : parseFloat(controle1Input.value.replace(/,/g, '.'));
  const controle2 = controle2Input.value.trim() === "" ? null : parseFloat(controle2Input.value.replace(/,/g, '.'));
  const controle3 = controle3Input.value.trim() === "" ? null : parseFloat(controle3Input.value.replace(/,/g, '.'));
  const synthese = syntheseInput.value.trim() === "" ? null : parseFloat(syntheseInput.value.replace(/,/g, '.'));
  ...
  if (weights === 0) {
    result.textContent = "Moyenne Sport:";
    result.classList.remove("error");
    result.classList.add("gradient-text");
    return;
  }
  const valuesToCheck = [];
  if (controle1 !== null) valuesToCheck.push(controle1);
  if (controle2 !== null) valuesToCheck.push(controle2);
  if (controle3 !== null) valuesToCheck.push(controle3);
  if (synthese !== null) valuesToCheck.push(synthese);
...
if (document.getElementById("controle1sport")) {
  document.getElementById("controle1sport").addEventListener("input", calculateMoyennesport);
}
if (document.getElementById("controle2sport")) {
  document.getElementById("controle2sport").addEventListener("input", calculateMoyennesport);
}
if (document.getElementById("controle3sport")) {
  document.getElementById("controle3sport").addEventListener("input", calculateMoyennesport);
}
...
if (document.getElementById("resBtn")) {
  document.getElementById("resBtn").addEventListener("click", () => {
    ...
    const sport = getNumberFromText("moyensport");
    
    const totalCoefficients = 16.5 ;
    const total = (
      arabe * 1 +
      anglais * 1.5 +
      francais * 1.5 +
...
function calculateMoyenneScoreTech() {
  try {
    ...
    const inputsToValidate = [technique, math, physique, anglais, francais, arabe,  
                             philosophie, informatique, optionValue];
    ...
    let totalCoefficients = 13.5; // الأساس بدون الرياضة (3+2+3+1+1+1+1+1+2)
    let totalMoyenne =  (technique* 3) + (math * 1) + (physique * 1) + 
                      (anglais * 1.5) + (francais * 1.5) + arabe+ (philosophie * 1.5) + 
    ...
    let score = 4 * moyenne + 1.5 * technique  + 0.5 * physique + 0.5 * philosophie+ 
               francais + anglais+ informatique+ 0.5 * sport;
