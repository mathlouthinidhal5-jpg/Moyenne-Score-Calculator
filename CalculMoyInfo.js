...
function calculateMoyennetechnique() {
  // حقول الميكانيكا
  const tpMecaInput = document.getElementById("tpmeca");
  const controleMecaInput = document.getElementById("controlemeca");
  const syntheseMecaInput = document.getElementById("synthesemeca");
  
  // حقول الكهرباء
  const tpElecInput = document.getElementById("tpelc");
  const controleElecInput = document.getElementById("controleelec");
  const syntheseElecInput = document.getElementById("syntheseelec");
  
  const result = document.getElementById("moyentechnique");
  
  if (!result) return;

  // تحويل القيم إلى أرقام (مع التعامل مع القيم الفارغة والفاصلة)
  const parseValue = (input) => input && input.value.trim() !== "" ? parseFloat(input.value.replace(/,/g, '.')) : null;

  const tpMeca = parseValue(tpMecaInput);
  const controleMeca = parseValue(controleMecaInput);
  const syntheseMeca = parseValue(syntheseMecaInput);
  const tpElec = parseValue(tpElecInput);
  const controleElec = parseValue(controleElecInput);
  const syntheseElec = parseValue(syntheseElecInput);

  result.classList.remove("error", "shake", "gradient-text");

  // إذا كانت جميع الحقول فارغة
  if ([syntheseMeca, syntheseElec].every(n => n === null)) {
    result.textContent = "Moyenne S.T.I:";
    return;
  }

  // جمع جميع القيم للتحقق من أنها بين 0 و 20
  const allValues = [tpMeca, controleMeca, syntheseMeca, tpElec, controleElec, syntheseElec].filter(n => n !== null);

  if (allValues.some(n => n < 0 || n > 20)) {
  ...
  // حساب المجموع المرجح للأوزان
  let total = 0;
  let weights = 0;

  // معاملات الأوزان (يمكن تعديلها حسب الحاجة)
  if (syntheseMeca !== null) {
    total += syntheseMeca * 2; // وزن 2 للتوليف الميكانيكي
    weights += 2;
  }
  if (controleMeca !== null) {
    total += controleMeca; // وزن 1 للتحكم الميكانيكي
    weights += 1;
  }
  if (tpMeca !== null) {
    total += tpMeca * 1; // وزن 0.5 لـ TP الميكانيكي (مثال)
    weights += 1;
  }

  if (syntheseElec !== null) {
    total += syntheseElec * 2; // وزن 2 للتوليف الكهربائي
    weights += 2;
  }
  if (controleElec !== null) {
    total += controleElec; // وزن 1 للتحكم الكهربائي
    weights += 1;
  }
  if (tpElec !== null) {
    total += tpElec * 1; // وزن 0.5 لـ TP الكهربائي (مثال)
    weights += 1;
  }

  const moyenne = weights > 0 ? total / weights : 0;
  result.textContent = `Moyenne S.T.I: ${moyenne.toFixed(2)}`;
  result.classList.add("gradient-text");
}
...
function calculateMoyenneinformatique() {
  const oraleInput = document.getElementById("tpinfo");
  ...
  if (isNaN(synthese)) {
    result.textContent = "Moyenne Programmation:";
    result.classList.remove("error");
    result.classList.add("gradient-text");
    return;
  }
  ...
  const moyenne = (orale + controle + synthese * 2) / elementsCount;
  result.textContent = `Moyenne Programmation: ${moyenne.toFixed(2)}`;
  result.classList.remove("error");
  result.classList.add("gradient-text");
}

if (document.getElementById("tpinfo")) {
  document.getElementById("tpinfo").addEventListener("input", calculateMoyenneinformatique);
}
...
if (document.getElementById("resBtn")) {
  document.getElementById("resBtn").addEventListener("click", () => {
    ...
    const totalCoefficients = 17 + sportCoefficient;
    const total = (
      arabe * 1 +
      anglais * 1 +
      francais * 1 +
...
function calculateMoyenneScoreTech() {
  try {
    ...
    let totalCoefficients = 15; // الأساس بدون الرياضة (3+2+3+1+1+1+1+1+2)
    let totalMoyenne =  (technique * 3) + (math * 3) + (physique * 2) + 
                      anglais + francais + arabe + philosophie + 
                      (informatique * 3) + optionContribution ;
    ...
    let score = 4 * moyenne + 0.5 * technique + 1.5 * math + 0.5 * physique + 
               francais + anglais+ 1.5 * informatique;
