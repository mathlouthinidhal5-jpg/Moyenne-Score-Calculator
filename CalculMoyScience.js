...
function calculateMoyenneScoreTech() {
  try {
    ...
    let totalCoefficients = 16; // الأساس بدون الرياضة (3+2+3+1+1+1+1+1+2)
    let totalMoyenne =  (technique* 4) + (math * 3) + (physique * 4) + 
    ...
    let score = 4 * moyenne + 1.5 * technique + 1 * math + 1.5 * physique + 
               francais + anglais;
